...
class PracticeExamSolver:
    ...
    def compare_results(self, user_result, correct_code, dataset_url):
        try: exec(f"import pandas as pd"); exec(f"df = pd.read_csv('{dataset_url}')"); exec_globals = {'pd': pd, 'df': df, 'np': np, 'len': len}; exec_locals = {}; exec(correct_code, exec_globals, exec_locals); lines = correct_code.strip().split('\n'); last_line = lines[-1].strip(); correct_result = eval(last_line, exec_globals, exec_locals) if last_line and not last_line.startswith('#') else None; correct_result_str = str(correct_result) if correct_result else "실행 완료"; user_result_str = str(user_result) if user_result else ""
        ...
        except Exception as e: return False, f"정답 코드 실행 오류: {str(e)}"
    def show_question(self, question_num):
        if question_num < 1 or question_num > len(self.df): print("문제 번호가 올바르지 않습니다."); return
        self.current_question = question_num; row_idx = question_num - 1; row = self.df.iloc[row_idx]
        try: problem_type = str(row.get('문제유형', '데이터분석')).strip(); question = str(row['문제']).strip(); dataset_url = str(row['데이터셋URL']).strip(); code_template = str(row['코드템플릿']).strip(); correct_code = str(row['정답코드']).strip(); explanation = str(row['해설']).strip(); difficulty = str(row.get('난이도', '초급')).strip()
        except KeyError as e: print(f"⚠️ 오류: 컬럼을 찾을 수 없습니다: {e}"); print(f"사용 가능한 컬럼: {row.index.tolist()}"); return
        ...
        code_textarea = widgets.Textarea(value=self.user_answers.get(question_num, ''), placeholder=code_template if code_template else '여기에 코드를 작성하세요...', description='코드 작성:', layout=widgets.Layout(width='100%', height='200px'), style={'description_width': 'initial'})
        ...
        def check_answer(b):
            ...
            if error: 
                with result_output: clear_output(); print(f"❌ 실행 오류:\n{error}")
                return
    ...
    def show_results(self):
        ...
        for idx, row in self.df.iterrows():
            question_num = idx + 1; user_code = self.user_answers.get(question_num, '미작성'); user_result = self.user_results.get(question_num, '미실행')
            if user_code != '미작성' and user_result != '미실행':
                try: correct_code = str(row['정답코드']).strip(); dataset_url = str(row['데이터셋URL']).strip(); is_correct, _ = self.compare_results(user_result, correct_code, dataset_url)
                if is_correct: correct_count += 1; status = "✅ 정답"; bg_color = "#c8e6c9"
                else: status = "❌ 오답"; bg_color = "#ffcdd2"
                except: status = "⚠️ 확인 필요"; bg_color = "#fff9c4"
            else: status = "⏸ 미완료"; bg_color = "#e0e0e0"
